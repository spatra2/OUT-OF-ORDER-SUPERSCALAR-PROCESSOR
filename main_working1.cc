...
void Execute()
{
	...
	for (vector<instruction>::iterator it = execute_list.begin() ; it != execute_list.end(); ++it)
        {
		if((it->operation_type == 0 && it->ex_latency == 1) || (it->operation_type == 1 && it->ex_latency == 2) || (it->operation_type == 2 && it->ex_latency ==5))
		{
			it->state=WB;
...
int main(int argc, char *argv[]) {
	...
	tracefile = 	  argv[8] ;
		...
		Fetch(file,n);
		cycles = cycles + 1;	
		printf ("Check_point:\t%d\t%d\t%d\t%d\t%d\t%d\n", cycles, tag_num, signed(fake_rob.size()), signed(dispatch_list.size()), signed(issue_list.size()), signed(execute_list.size()));
